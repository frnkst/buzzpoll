...
use crate::model::{VoteRequest, Poll};
...
#[post("/poll")]
async fn create_poll(poll: web::Json<Poll>, data: web::Data<AppState>) -> Result<HttpResponse, Error> {
...
#[post("/vote")]
async fn vote(vote_request: web::Json<VoteRequest>, data: web::Data<AppState>) -> Result<HttpResponse, Error> {
    let mut all_polls = data.all_polls.lock().unwrap();
